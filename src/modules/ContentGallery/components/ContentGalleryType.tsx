...
function ContentGalleryType({ title, image, onPress }: Props) {
  ...
  return (
    <Pressable onPress={onPress} onPressIn={shrinkContainer} onPressOut={expandContainer}>
      ...
      >
        <Image h="270px" w="full" source={image} alt={title} rounded={4} />

        <Box position="absolute" bottom={0} h="46px" w="full">
          <Text color="white" m={3} zIndex={1}>
            {title}
          </Text>
...
export default ContentGalleryType;
